...
plot <- ggplot(df_subset, aes(output_len, fill = label)) +
  ...
  geom_label(
    data = df_agg_subset, mapping = aes(label = sprintf("x̄ = %s\nσ = %s", round(avg), round(std)), color = label, x = 20, y = Inf),
    vjust = 1.1, hjust = 0, fill = "white",
    family = "Overpass",
    size = 2, lineheight = 1.0, label.padding = unit(0.20, "lines"), label.size = 0.3,
  ) +
...
plot <- ggplot(df_subset, aes(output_len, fill = label)) +
  geom_histogram(binwidth = 10) +
  geom_label(
    data = df_agg_subset, mapping = aes(label = sprintf("x̄ = %s\nσ = %s\np = %s", round(avg), round(std), round(pval, 3)), color = label, x = 20, y = Inf),
    vjust = 1.1, hjust = 0, fill = "white",
    family = "Overpass",
    size = 2, lineheight = 1.0, label.padding = unit(0.20, "lines"), label.size = 0.3,
  ) +
